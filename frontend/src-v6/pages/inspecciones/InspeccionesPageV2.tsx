...
import { INSPECTION_ACTOR_LABELS, INSPECTION_STATE_COLORS, INSPECTION_STATE_LABELS, inspectionDate, inspectionErrorMessage, inspectionRelevantDate, isTrainingActNumber } from './inspectionPresentation';
...
const InspeccionesPageV2: React.FC = () => {
  ...
  return <div className="space-y-5 animate-fade-in">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-col gap-2 sm:flex-row"><Button className="w-full sm:w-auto" variant="outline" leftIcon={<FileDown size={17} />} isLoading={exporting} disabled={offline || inspections.isError} onClick={exportList}>Exportar listado</Button><Button className="w-full sm:w-auto" leftIcon={<Plus size={17} />} disabled={offline || inspections.isError} onClick={() => setShowCreate(true)}>Nueva inspección</Button></div>
    </header>
    ...
    <div className="grid grid-cols-3 gap-2 sm:gap-3">
      <button onClick={() => { setState(state === 'EN_CAMPO' ? '' : 'EN_CAMPO'); setPage(1); }} className={`min-h-24 rounded-xl border bg-white p-3 text-left transition sm:min-h-0 sm:p-4 ${state === 'EN_CAMPO' ? 'border-primary-500 ring-2 ring-primary-100' : 'border-neutral-200 hover:border-primary-300'}`}><p className="text-xs leading-tight text-neutral-500 sm:text-sm">En campo</p><p className="mt-2 text-2xl font-bold text-primary-700">{totalsUnknown ? '—' : inField}</p></button>
      <button onClick={() => { setState(state === 'EN_REVISION' ? '' : 'EN_REVISION'); setPage(1); }} className={`min-h-24 rounded-xl border bg-white p-3 text-left transition sm:min-h-0 sm:p-4 ${state === 'EN_REVISION' ? 'border-amber-500 ring-2 ring-amber-100' : 'border-neutral-200 hover:border-amber-300'}`}><p className="text-xs leading-tight text-neutral-500 sm:text-sm">Pendientes de revisión</p><p className="mt-2 text-2xl font-bold text-amber-700">{totalsUnknown ? '—' : pendingReview}</p></button>
      <div className="min-h-24 rounded-xl border border-neutral-200 bg-white p-3 sm:min-h-0 sm:p-4"><p className="text-xs leading-tight text-neutral-500 sm:text-sm">Con plazo abierto</p><p className="mt-2 text-2xl font-bold text-blue-700">{totalsUnknown ? '—' : deadlines}</p></div>
    </div>

    <div className="grid gap-3 rounded-xl border border-neutral-200 bg-white p-3 sm:grid-cols-[minmax(240px,1fr)_220px_190px]">
      <label className="relative"><span className="sr-only">Buscar inspección</span><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" size={17} /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Buscar por expediente, acta o actor" className="h-11 w-full rounded-lg border border-neutral-300 bg-white pl-10 pr-3 text-sm text-neutral-900 focus:border-primary-600 focus:outline-none focus:ring-2 focus:ring-primary-100" /></label>
    ...
    <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white">
      ...
      {inspections.isLoading ? <p className="p-8 text-center text-sm text-neutral-500">Cargando inspecciones…</p> : inspections.isError && rows.length === 0 ? <div role="alert" className="p-8 text-center"><p className="font-semibold text-error-800">No se pudo cargar el listado</p><p className="mt-1 text-sm text-neutral-600">No significa que no haya inspecciones. Comprobá la conexión y volvé a intentar.</p><Button variant="outline" className="mt-4" onClick={() => void inspections.refetch()}>Reintentar</Button></div> : rows.length === 0 ? <div className="p-10 text-center"><ClipboardCheck className="mx-auto text-neutral-300" size={40} /><p className="mt-3 font-semibold text-neutral-800">{offline ? 'No hay copias locales para estos filtros' : 'No hay inspecciones para estos filtros'}</p><p className="mt-1 text-sm text-neutral-500">{offline ? 'Con conexión, abrí el expediente una vez para dejarlo disponible en este dispositivo.' : 'Ajustá la búsqueda o creá un nuevo expediente.'}</p></div> : rows.map((inspection) => {
        ...
        return <Link key={inspection.id} to={inspectionRoute + (resume?.hash || '')} aria-label={`${resume ? 'Retomar' : 'Abrir'} expediente ${inspection.numero}${resume ? ` en ${resume.label}` : ''}`} data-testid={`inspection-row-${inspection.id}`} className="group grid w-full cursor-pointer grid-cols-1 gap-3 border-b border-neutral-100 p-4 text-left transition-colors last:border-0 hover:bg-primary-50/40 active:bg-primary-100/60 focus-visible:relative focus-visible:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500 lg:grid-cols-[0.92fr_1.35fr_1fr_1fr_auto] lg:items-center lg:gap-4 lg:px-5">
          <div><div className="flex flex-wrap items-center justify-between gap-2 lg:justify-start"><span className="font-extrabold text-[#10213A]">{inspection.numero}</span>{isTrainingActNumber(inspection.numeroActa) && <Badge color="warning">Capacitación</Badge>}<Badge className="lg:hidden" color={INSPECTION_STATE_COLORS[inspection.estado] || 'neutral'}>{INSPECTION_STATE_LABELS[inspection.estado]}</Badge></div><p className="mt-1 text-xs text-neutral-500">Acta {inspection.numeroActa || 'sin numerar'}</p><p className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-neutral-700"><CalendarDays size={14} className="text-neutral-400" />{inspectionDate(dateValue)}</p>{resume && <p className="mt-1 line-clamp-2 text-xs font-semibold text-primary-800">Retomar: {resume.label}</p>}</div>
