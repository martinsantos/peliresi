...
- Keystore nueva creada en `android-twa/secrets/rptrazar-release.keystore`.
...
- DNS real: `rptrazar.mendoza.gov.ar` -> `diclb.mendoza.gov.ar` -> `192.168.192.135`.
- `192.168.192.135:22` y `:443` responden.
- SSH a `ubuntu@192.168.192.135`, `root@192.168.192.135` y `ubuntu@rptrazar.mendoza.gov.ar` rechaza autenticacion.
- La clave documentada `~/.ssh/ambiente.pem` tambien fue rechazada.
...
1. Obtener acceso al host real `192.168.192.135` o pedir a infraestructura que publique `frontend/public/.well-known/assetlinks.json` en `/var/www/sitrep/.well-known/assetlinks.json` del servidor real.
