...
import { AlertTriangle, Camera, CheckCircle2, ChevronDown, ChevronUp, CircleMinus, ClipboardCheck, FileAudio, FileText, Fingerprint, ShieldCheck, XCircle } from 'lucide-react';
...
export function InspectionReport({ inspection }: { inspection: Inspection }) {
  ...
  const annulledEvidence = inspection.evidencias.filter((item) => item.anuladaAt);
    ...
    <section className="overflow-hidden rounded-2xl border border-neutral-200 bg-white">
      ...
      <div className="space-y-7 px-4 py-5 sm:px-6">
        ...
        <div>
          ...
          {photos.length > 0 ? <><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{(showAllPhotos ? photos : photos.slice(0, 6)).map((photo, index) => <figure key={photo.id} className="overflow-hidden rounded-xl border border-neutral-200"><InspectionEvidenceImage inspectionId={inspection.id} evidenceId={photo.id} alt={photo.descripcion || photo.nombreOriginal} preview className="aspect-[4/3] w-full object-cover" /><figcaption className="p-3"><p className="break-all text-sm font-bold text-[#10213A]">{String(index + 1).padStart(2, '0')}. {photo.nombreOriginal}</p><p className="mt-1 text-xs leading-relaxed text-neutral-600">{photo.descripcion || `Capturada ${new Date(photo.capturadaAt).toLocaleString('es-AR')}`}</p></figcaption></figure>)}</div>{photos.length > 6 && <button type="button" onClick={() => setShowAllPhotos((value) => !value)} className="mt-3 inline-flex min-h-10 items-center gap-1 rounded-lg px-3 text-xs font-bold text-primary-800 hover:bg-primary-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500">{showAllPhotos ? <ChevronUp size={16} /> : <ChevronDown size={16} />}{showAllPhotos ? 'Mostrar galería resumida' : `Ver las ${photos.length} fotografías`}</button>}</> : <div className="flex min-h-28 items-center justify-center rounded-xl border border-dashed border-neutral-300 bg-neutral-50 text-sm text-neutral-500"><Camera size={18} className="mr-2" />Aún no hay fotografías vigentes.</div>}
          {activeEvidence.some((item) => item.tipo !== 'FOTO') && <div className="mt-3 flex flex-wrap gap-2">{activeEvidence.filter((item) => item.tipo !== 'FOTO').map((item) => <span key={item.id} className="inline-flex max-w-full min-w-0 items-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 text-xs font-semibold text-neutral-700">{item.tipo === 'AUDIO' ? <FileAudio className="shrink-0" size={16} /> : <FileText className="shrink-0" size={16} />}<span className="min-w-0 break-all">{item.nombreOriginal}</span></span>)}</div>}
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-neutral-200 bg-neutral-50">
          ...
          {inspection.evidencias.length === 0 ? <p className="px-4 py-4 text-sm text-neutral-500">No hay archivos cuya integridad verificar.</p> : <div className="divide-y divide-neutral-200">{inspection.evidencias.map((evidence) => <div key={evidence.id} className={`grid min-w-0 gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start ${evidence.anuladaAt ? 'bg-neutral-100' : ''}`}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className={`break-all text-sm font-bold ${evidence.anuladaAt ? 'text-neutral-500 line-through' : 'text-[#10213A]'}`}>{evidence.nombreOriginal}</p>{evidence.anuladaAt && <span className="rounded-full bg-neutral-200 px-2 py-0.5 text-[10px] font-bold text-neutral-700">Anulada · archivo preservado</span>}</div><p className="mt-0.5 text-xs text-neutral-600">Captura {new Date(evidence.capturadaAt).toLocaleString('es-AR')} · Recepción {new Date(evidence.createdAt).toLocaleString('es-AR')}{evidence.creadoPor ? ` · ${evidence.creadoPor.nombre} ${evidence.creadoPor.apellido || ''}` : ''}</p>{evidence.descripcion && <p className="mt-1 whitespace-pre-line text-xs leading-relaxed text-neutral-700"><span className="font-bold">Descripción:</span> {evidence.descripcion}</p>}{evidence.transcripcion && <p className="mt-1 whitespace-pre-line text-xs leading-relaxed text-neutral-700"><span className="font-bold">Transcripción:</span> {evidence.transcripcion}</p>}{evidence.anuladaAt && <p className="mt-1 text-xs leading-relaxed text-neutral-700"><span className="font-bold">Motivo:</span> {evidence.motivoAnulacion || 'Sin motivo informado'}</p>}</div><p title={evidence.sha256 || 'Huella no disponible'} className="flex max-w-full items-center gap-1.5 rounded-md border border-neutral-200 bg-white px-2 py-1 font-mono text-[10px] font-semibold text-primary-800"><Fingerprint size={13} className="shrink-0" />{evidence.sha256 ? `${evidence.sha256.slice(0, 16)}…` : 'Sin huella'}</p></div>)}</div>}
        </div>
