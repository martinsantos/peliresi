...
export function InspectionComparisonPanel({ inspectionId, comparisons, editable, onChange, onEvidence }: {
  ...
  useEffect(() => {
    if (!expandedGroup && groups.length > 0) setExpandedGroup(groups[0]);
    if (expandedGroup && !groups.includes(expandedGroup)) setExpandedGroup(groups[0] || null);
      ...
      {groups.map((group) => {
        ...
        return (
          <div key={group} className="border-b border-neutral-200 last:border-0">
            <button type="button" aria-expanded={expandedGroup === group} onClick={() => setExpandedGroup((value) => value === group ? null : group)} className="flex min-h-14 w-full items-center justify-between gap-3 bg-neutral-50 px-4 text-left sm:px-6">
              ...
              {expandedGroup === group ? <ChevronUp size={18} className="shrink-0" /> : <ChevronDown size={18} className="shrink-0" />}
            </button>
            {expandedGroup === group && rows.map((row) => (
