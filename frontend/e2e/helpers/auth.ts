...
const AUTHENTICATED_SHELL_TEXT = /Inicio|Buenos d.as|Accesos R.pidos|Centro de Control|Manifiestos/i;

async function waitForAuthenticatedShell(page: Page, timeout = 25000) {
  return page.waitForFunction(
    (shellPattern) => {
      const bodyText = document.body?.innerText || '';
      return (
        !!document.querySelector('aside, main, nav, [role="navigation"]') ||
        new RegExp(shellPattern, 'i').test(bodyText)
      );
    },
    AUTHENTICATED_SHELL_TEXT.source,
    { timeout },
  );
}
...
export async function loginWithCredentials(page: Page, options: LoginOptions) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const reachedAuthOutcome = await page.waitForFunction(
      ...
      [RATE_LIMIT_TEXT.source, AUTHENTICATED_SHELL_TEXT.source],
      { timeout: 25000 },
    ...
    if (!rateLimited) {
      await waitForAuthenticatedShell(page, 20000);
