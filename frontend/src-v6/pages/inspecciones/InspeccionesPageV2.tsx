...
import { INSPECTION_ACTOR_LABELS, INSPECTION_STATE_COLORS, INSPECTION_STATE_LABELS, inspectionDate, inspectionRelevantDate } from './inspectionPresentation';
...
const InspeccionesPageV2: React.FC = () => {
  ...
  const createInspection = async () => {
    ...
    } catch (error: any) { toast.error('No se pudo crear', error?.response?.data?.message || 'Revise los datos e intente nuevamente.'); }
  };
  ...
  return <div className="space-y-5 animate-fade-in">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="grid grid-cols-2 gap-2 sm:flex"><Button variant="outline" leftIcon={<FileDown size={17} />} isLoading={exporting} onClick={exportList}>Exportar listado</Button><Button leftIcon={<Plus size={17} />} onClick={() => setShowCreate(true)}>Nueva inspección</Button></div>
    </header>
