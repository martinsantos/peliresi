...
export async function buildDeclaredInspectionSnapshot(
  ...
  if (tipoActor === 'GENERADOR') {
    const actor = await db.generador.findUniqueOrThrow({
      where: { id: actorId },
      include: { documentosRegulatorios: { select: { estado: true, tipo: true, vigenteHasta: true } } },
    });
    fields = [
      ...
      field('DOC-VIGENTES', 'Documentación', 'Documentos regulatorios aprobados', 'generador.documentosRegulatorios', actor.documentosRegulatorios.filter((d) => d.estado === 'APROBADO').length, 110),
    ];
    ...
    const actor = await db.transportista.findUniqueOrThrow({
      ...
      include: {
        ...
        documentosRegulatorios: { select: { estado: true } },
      },
    });
    fields = [
      ...
      field('DOC-VIGENTES', 'Documentación', 'Documentos regulatorios aprobados', 'transportista.documentosRegulatorios', actor.documentosRegulatorios.filter((d) => d.estado === 'APROBADO').length, 110),
    ];
  } else {
    ...
    fields = [
      ...
      field('DOC-VIGENTES', 'Documentación', 'Documentos regulatorios aprobados', 'operador.documentosRegulatorios', actor.documentosRegulatorios.filter((d) => d.estado === 'APROBADO').length, 120),
    ];
  }
