...
const InspeccionExpedientePage: React.FC = () => {
  ...
  const transition = async (next: InspectionState) => { if (!isOnline) return toast.warning('Conexión requerida', 'El cierre requiere confirmación del servidor.'); try { await transitionMutation.mutateAsync(next); toast.success('Estado actualizado', next === 'NOTIFICADA' ? 'Acta aprobada; no se envió correo externo.' : LABELS[next]); } catch (error: unknown) { toast.error('Acción rechazada', inspectionErrorMessage(error, 'No se pudo cambiar el estado.')); } };
  const upload = async (file?: File, fields?: { comparacionId?: string; eventoId?: string; itemId?: string; descripcion?: string }, persistDraft = false) => {
    ...
    try {
      ...
      toast.error('Evidencia rechazada', inspectionErrorMessage(error, 'No se pudo cargar.'));
    } finally {
