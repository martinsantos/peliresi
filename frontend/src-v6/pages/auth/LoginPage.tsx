...
const QUICK_LOGIN_META: Record<number, { label: string; sublabel: string; color: string }> = {
  1: { label: 'Administrador', sublabel: 'DGFA', color: 'bg-primary-500' },
  5: { label: 'Generador', sublabel: 'Química Mendoza', color: 'bg-purple-500' },
  13: { label: 'Transportista', sublabel: 'Transportes Andes', color: 'bg-orange-500' },
  19: { label: 'Operador', sublabel: 'Tratamiento Residuos', color: 'bg-blue-500' },
};
...
const LoginPage: React.FC = () => {
  ...
  const quickLogins = useMemo(() => Object.entries(DEMO_CREDENTIALS).map(([id, credentials]) => ({
    id: Number(id),
    ...credentials,
    ...QUICK_LOGIN_META[Number(id)],
  ...
  const handleQuickLogin = (q: typeof quickLogins[number]) => {
    setSelectedUserId(q.id);
    setEmail(q.email);
        ...
        <>
          <div className="mb-6 md:mb-8">
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-3 stagger-children">
              {quickLogins.map((q) => {
                const isSelected = selectedUserId === q.id;
                return (
                  <button
                    key={q.id}
